    ...
    def get_historical_data(self, days: int = 30) -> pd.DataFrame:
        ...
        try:
            ...
            if not historical_data.empty:
                ...
                logger.warning("⚠️ No IBKR data available, using silver layer historical data")
                return self._get_silver_layer_data(days)
                
        except Exception as e:
            logger.error(f"❌ Historical data error: {e}")
            return self._get_silver_layer_data(days)
    
    def _get_silver_layer_data(self, days: int) -> pd.DataFrame:
        """Get REAL historical data from silver layer data warehouse"""
        try:
            connector = SilverLayerDataConnector()
            
            # Get historical records (days * 24 for hourly data)
            ...
            if not historical_data.empty:
                logger.info(f"✅ Retrieved {len(historical_data)} REAL historical data points from silver layer")
                # Standardize column names for compatibility
                data_formatted = historical_data.rename(columns={
                    'Datetime': 'timestamp',
                    'open': 'open',
                    'high': 'high', 
                    'low': 'low',
                ...
                logger.error("❌ No data available from silver layer")
                return pd.DataFrame()
                
        except Exception as e:
            logger.error(f"❌ Silver layer data error: {e}")
            return pd.DataFrame()
    
    def _generate_fallback_data(self, days: int) -> pd.DataFrame:
        """DEPRECATED: No longer using simulated data - Real data only from silver layer"""
        logger.error("❌ DEPRECATED: Simulated data generation removed - Use silver layer only")
        return pd.DataFrame()
