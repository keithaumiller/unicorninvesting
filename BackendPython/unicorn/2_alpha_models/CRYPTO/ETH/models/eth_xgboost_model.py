#!/usr/bin/env python3
"""
ETH XGBoost Model - Multi-Interval Implementation  
...
class ETHXGBoostModel:
    """XGBoost prediction model for ETH"""
    ...
    def load_data(self) -> pd.DataFrame:
        ...
        try:
            if self.category == "crypto":
                filepath = self.data_path / f"yahoo_finance_assets/processed_data/crypto/ETH_silver_{self.interval}_latest.csv"
            else:
                filepath = self.data_path / f"yahoo_finance_assets/processed_data/forex/ETH_silver_{self.interval}_latest.csv"
                
            if not filepath.exists():
                raise FileNotFoundError(f"Data file not found: {filepath}")
                
            df = pd.read_csv(filepath, index_col=0, parse_dates=True)
            logging.info(f"Loaded ETH {self.interval} data: {df.shape[0]} records × {df.shape[1]} features")
            return df
    ...
    def save_model(self, filepath: str = None):
        """Save trained model"""
        if not filepath:
            filepath = f"ETH_xgboost_{self.interval}_model.pkl"
...
def main():
    ...
    parser = argparse.ArgumentParser(description='ETH XGBoost Model')
    ...
    # Train model
    model = ETHXGBoostModel(interval=args.interval)
    result = model.train_model(test_size=args.test_size)
    
    if result['status'] == 'success':
        print(f"✅ ETH XGBoost {args.interval} model training successful!")
