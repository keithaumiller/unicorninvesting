#!/usr/bin/env python3
"""
BTC Ensemble Model - Multi-Interval Implementation
...
from btc_prophet_model import BTCProphetModel
from btc_xgboost_model import BTCXGBoostModel

from sklearn.metrics import mean_absolute_error, mean_squared_error, r2_score
from sklearn.linear_model import LinearRegression

class BTCEnsembleModel:
    ...
    def __init__(self, interval: str = "1d"):
        ...
        self.prophet_model = BTCProphetModel(interval=interval)
        self.xgboost_model = BTCXGBoostModel(interval=interval)
    ...
    def train_component_models(self) -> dict:
        ...
        try:
            # Train Prophet model
            logging.info(f"Training Prophet model for BTC {self.interval}")
            ...
            # Train XGBoost model
            logging.info(f"Training XGBoost model for BTC {self.interval}")
    ...
    def save_ensemble(self, filepath: str = None):
        ...
        if not filepath:
            filepath = f"BTC_ensemble_{self.interval}_model.pkl"
...
def main():
    ...
    parser = argparse.ArgumentParser(description='BTC Ensemble Model')
    ...
    # Train ensemble
    ensemble = BTCEnsembleModel(interval=args.interval)
    result = ensemble.train_ensemble(method=args.method)
    
    if result['status'] == 'success':
        print(f"✅ BTC Ensemble {args.interval} model training successful!")
