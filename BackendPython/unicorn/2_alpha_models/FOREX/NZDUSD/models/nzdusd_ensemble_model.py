#!/usr/bin/env python3
"""
NZDUSD Ensemble Model - Multi-Interval Implementation
...
from nzdusd_prophet_model import NZDUSDProphetModel
from nzdusd_xgboost_model import NZDUSDXGBoostModel

from sklearn.metrics import mean_absolute_error, mean_squared_error, r2_score
from sklearn.linear_model import LinearRegression

class NZDUSDEnsembleModel:
    """Ensemble model combining Prophet and XGBoost for NZDUSD"""
    ...
    def __init__(self, interval: str = "1d"):
        ...
        self.prophet_model = NZDUSDProphetModel(interval=interval)
        self.xgboost_model = NZDUSDXGBoostModel(interval=interval)
    ...
    def train_component_models(self) -> dict:
        ...
        try:
            # Train Prophet model
            logging.info(f"Training Prophet model for NZDUSD {self.interval}")
            ...
            # Train XGBoost model
            logging.info(f"Training XGBoost model for NZDUSD {self.interval}")
    ...
    def save_ensemble(self, filepath: str = None):
        ...
        if not filepath:
            filepath = f"NZDUSD_ensemble_{self.interval}_model.pkl"
...
def main():
    ...
    parser = argparse.ArgumentParser(description='NZDUSD Ensemble Model')
    ...
    # Train ensemble
    ensemble = NZDUSDEnsembleModel(interval=args.interval)
    result = ensemble.train_ensemble(method=args.method)
    
    if result['status'] == 'success':
        print(f"✅ NZDUSD Ensemble {args.interval} model training successful!")
