#!/usr/bin/env python3
"""
AUDUSD XGBoost Model - Multi-Interval Implementation  
...
class AUDUSDXGBoostModel:
    """XGBoost prediction model for AUDUSD"""
    ...
    def load_data(self) -> pd.DataFrame:
        ...
        try:
            if self.category == "crypto":
                filepath = self.data_path / f"yahoo_finance_assets/processed_data/crypto/AUDUSD_silver_{self.interval}_latest.csv"
            else:
                filepath = self.data_path / f"yahoo_finance_assets/processed_data/forex/AUDUSD_silver_{self.interval}_latest.csv"
                
            if not filepath.exists():
                raise FileNotFoundError(f"Data file not found: {filepath}")
                
            df = pd.read_csv(filepath, index_col=0, parse_dates=True)
            logging.info(f"Loaded AUDUSD {self.interval} data: {df.shape[0]} records × {df.shape[1]} features")
            return df
    ...
    def save_model(self, filepath: str = None):
        """Save trained model"""
        if not filepath:
            filepath = f"AUDUSD_xgboost_{self.interval}_model.pkl"
...
def main():
    ...
    parser = argparse.ArgumentParser(description='AUDUSD XGBoost Model')
    ...
    # Train model
    model = AUDUSDXGBoostModel(interval=args.interval)
    result = model.train_model(test_size=args.test_size)
    
    if result['status'] == 'success':
        print(f"✅ AUDUSD XGBoost {args.interval} model training successful!")
