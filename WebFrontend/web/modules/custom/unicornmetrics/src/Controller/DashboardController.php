...
class DashboardController extends ControllerBase {
  ...
  public function leanSimulations() {
    ...
    $build = [
      '#markup' => $this->buildSimulationManagement(),
      '#attached' => [
        'library' => [
  ...
  private function buildSimulationManagement() {
    ...
    $available_portfolios = $this->getAvailablePortfolios();
    $current_portfolio = !empty($available_portfolios) ? $available_portfolios[0] : ['id' => 'none', 'name' => 'No Portfolio', 'description' => 'No portfolios found'];
    $available_simulations = !empty($available_portfolios) ? $this->getAvailableSimulations($current_portfolio['id']) : [];
    ...
    $content .= '
    <div class="portfolio-selector-container">
      <div class="portfolio-selector" id="admin-portfolio-selector">
        <label for="portfolio-select">Available Portfolios:</label>
        <select id="portfolio-select" name="portfolio" onchange="loadPortfolioSimulations()">
          <option value="">-- Select Portfolio --</option>';
    
    foreach ($available_portfolios as $portfolio) {
      $selected = ($portfolio['id'] === $current_portfolio['id']) ? 'selected' : '';
      $content .= '<option value="' . htmlspecialchars($portfolio['id']) . '" ' . $selected . '>'
                . htmlspecialchars($portfolio['name']) . ' (' . htmlspecialchars($portfolio['strategy'] ?? 'unknown') . ')</option>';
    }
    
    $content .= '
        </select>
        <div class="portfolio-info" id="portfolio-info">
          <h4>📊 ' . htmlspecialchars($current_portfolio['name']) . '</h4>
          <p>' . htmlspecialchars($current_portfolio['description'] ?? 'No description available') . '</p>
          <p><strong>Strategy:</strong> ' . htmlspecialchars($current_portfolio['strategy'] ?? 'unknown') . '</p>
          <p><strong>Simulations Available:</strong> ' . count($available_simulations) . '</p>
        </div>
      </div>
    </div>';
    ...
    $content .= '
    <script>
    function loadPortfolioSimulations() {
      ...
      if (selectedPortfolio) {
        ...
        // Here you would normally make an AJAX call to load simulations
        // For now, we\'ll just update the display
        portfolioInfo.innerHTML = "<h4>📊 " + portfolioName + "</h4><p>Loading simulations...</p>";
        
        // Reload the page to show simulations for selected portfolio
        // In a production system, this would be an AJAX call
  ...
  /**
   * Get available portfolios from 4_portfolios directory.
   */
  private function getAvailablePortfolios() {
    ...
    $directories = scandir($portfolios_path);
    foreach ($directories as $dir) {
      if ($dir === '.' || $dir === '..' || $dir === 'utilities' || $dir === '__pycache__') {
        continue;
      }
      
      $portfolio_path = $portfolios_path . '/' . $dir;
      if (is_dir($portfolio_path)) {
        $portfolios[] = [
          'id' => $dir,
          'name' => $dir,
          'description' => 'Portfolio located in 4_portfolios/' . $dir,
          'strategy' => $this->detectPortfolioStrategy($portfolio_path),
          'path' => $portfolio_path
        ];
      }
    }
  ...
  /**
   * Get available simulations for a specific portfolio.
   */
  private function getAvailableSimulations($portfolio_id) {
    $simulations = [];
    $simulations_path = "/workspaces/unicorninvesting/BackendPython/unicorn/4_portfolios/{$portfolio_id}/simulations/backtests";
    
    if (!is_dir($simulations_path)) {
      return $simulations;
    }
    
    $directories = scandir($simulations_path);
    foreach ($directories as $dir) {
      if ($dir === '.' || $dir === '..') {
        continue;
      }
      ...
      if (is_dir($simulation_path)) {
        $simulation_data = $this->analyzeSimulation($simulation_path, $dir, $portfolio_id);
  ...
  /**
   * Detect portfolio strategy from directory contents.
   */
  private function detectPortfolioStrategy($portfolio_path) {
    // Look for strategy indicators in file names or config files
    if (file_exists($portfolio_path . '/trading_algorithms')) {
      return 'algorithmic';
    } elseif (file_exists($portfolio_path . '/risk_algorithms')) {
      return 'risk-managed';
    }
    return 'unknown';
  }
  ...
  /**
   * Analyze a simulation directory to extract metadata.
   */
  private function analyzeSimulation($simulation_path, $simulation_id, $portfolio_id) {
    $simulation_data = [
      'id' => $simulation_id,
      'name' => $simulation_id,
    ...
    if (file_exists($simulation_path . '/error.json')) {
      $simulation_data['status'] = 'Failed';
    } elseif (file_exists($simulation_path . '/lean_config.json')) {
      $simulation_data['status'] = 'Completed';
    } else {
      $simulation_data['status'] = 'In Progress';
    }
    
    // Extract creation date from directory name if possible
    if (preg_match('/(\d{8})/', $simulation_id, $matches)) {
      $date_str = $matches[1];
      $simulation_data['created'] = date('M j, Y', strtotime($date_str));
    }
